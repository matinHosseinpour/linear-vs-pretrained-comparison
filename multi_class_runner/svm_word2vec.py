...
def preprocess_text(text):
    ...
    return stems  # Return list of stems (tokens)

def compute_average_word2vec_vector(tokens, model, vector_size):
...
for file in files:
    ...
    vector_size = 100  # You can adjust this size
    word2vec_model = Word2Vec(sentences=texts, vector_size=vector_size, window=5, min_count=1, workers=4)

    # 5. Convert texts to averaged Word2Vec vectors
    X = np.array([compute_average_word2vec_vector(text, word2vec_model, vector_size) for text in texts])
    y = np.array(labels)

    # 6. Define the SVM classifier
    svm_classifier = SVC(kernel='linear', random_state=42)
    if file['class_weight']:
        values = {int(k): int(v) for k, v in data['sentiment'].value_counts().to_dict().items()}
        major = max(values.values())
        for key in values:
            values[key] = major // values[key]
        svm_classifier = SVC(kernel='linear', class_weight=values, random_state=42)

    # 7. Implement k-Fold Cross-Validation and Collect Metrics
    k = 5
    ...
    # 9. Output Metrics to a JSON File
    output_data = {
    ...
    model_name = 'svm_word2vec.json'
    output_path = '../models_results/' + file['dataset'] + '/svm/'
    if file['pre_process']:
        output_path += 'pre_'
    else :
        output_path += ''
